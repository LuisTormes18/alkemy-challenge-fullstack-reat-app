...
import { FetchGetApi } from "./../helpers/FetchGetApi";
import useAuth from "./../Hooks/useAuth";
import FetchDeleteApi from "./../helpers/FetchDeleteApi";
import FetchUpdateApi from "./../helpers/FetchUpdateApi";
...
function RecordProvider({ children }) {
  ...
  function getBudget(){
    FetchGetApi(`${url_get_total_budget}${auth.getUserId()}`,auth.getToken()).then(data=>{
      setBudget(data);
    });
  }
  ...
  function newRecord(values){
     FetchPostApi(url_new_record, values,auth.getToken()).then(result=>{
